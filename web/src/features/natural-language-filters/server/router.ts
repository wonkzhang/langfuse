      ...
      try {
        ...
        if (
          ...
          throw new TRPCError({
            code: "PRECONDITION_FAILED",
            message:
              "Tedi AI filters environment variables not configured. Please set LANGFUSE_AI_FEATURES_PUBLIC_KEY and LANGFUSE_AI_FEATURES_SECRET_KEY variables.",
          });
        }
        ...
        if (!env.LANGFUSE_AI_FEATURES_PROJECT_ID) {
          throw new TRPCError({
            ...
            message: "Tedi AI Features not configured.",
          });
        }
