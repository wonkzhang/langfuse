...
Complete guide to middleware patterns in Tedi's Next.js + tRPC architecture.
...
tRPC middleware in Tedi is composable and type-safe. Each middleware enriches the context and provides guarantees to subsequent middleware.
...
Propagates OpenTelemetry context with Tedi-specific baggage:
...
Tedi exports composed procedures with middleware chains:
...
async function verifyAdminApiKeyAuth(req: NextApiRequest) {
  ...
  if (env.NEXT_PUBLIC_LANGFUSE_CLOUD_REGION) {
    throw {
      status: 403,
      message: "Admin API key auth not available on Tedi Cloud",
    };
  }
...
All requests (tRPC and public API) propagate OpenTelemetry context with Tedi-specific baggage.
...
// Create context with Tedi baggage
const ctx = contextWithLangfuseProps({
  headers: req.headers,
  userId: session?.user?.id,
