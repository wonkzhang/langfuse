...
Complete guide to database access patterns in Tedi using PostgreSQL (Prisma ORM) and ClickHouse (direct client).
...
Tedi uses a **dual database architecture**:
...
- Tedi is multi-tenant - each project's data must be isolated
...
Tedi uses repositories in `packages/shared/src/server/repositories/` for complex data access patterns.
